...
def UI_box(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img - OPTIMIZADO
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    cv2.rectangle(img, c1, c2, color, thickness=tl, lineType=cv2.LINE_4)  # Cambio: LINE_AA → LINE_4
    if label:
        tf = max(tl - 1, 1)  # font thickness
        t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
        # Cambio: draw_border() reemplazado con rectangle simple
...
def draw_boxes(img, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    # OPTIMIZADO: Contadores dibujados UNA SOLA VEZ fuera del loop principal
    # Display Count in top right corner
    for idx, (key, value) in enumerate(object_counter1.items()):
        cnt_str = str(key) + ":" + str(value)
        cv2.rectangle(img, (width - 500, 15), (width, 45), [85, 45, 255], -1, cv2.LINE_4)
        cv2.putText(img, 'Vehicles Entering', (width - 490, 35), 0, 0.7, [225, 255, 255], thickness=1, lineType=cv2.LINE_4)
        cv2.rectangle(img, (width - 250, 55 + (idx*35)), (width - 50, 85 + (idx*35)), [85, 45, 255], -1, cv2.LINE_4)
        cv2.putText(img, cnt_str, (width - 240, 75 + (idx*35)), 0, 0.8, [255, 255, 255], thickness=1, lineType=cv2.LINE_4)

    for idx, (key, value) in enumerate(object_counter.items()):
        cnt_str1 = str(key) + ":" + str(value)
        cv2.rectangle(img, (10, 15), (490, 45), [85, 45, 255], -1, cv2.LINE_4)
...
class DetectionPredictor(BasePredictor):
    ...
    # Filtro de clases: 0=person, 1=bicycle, 2=car, 3=motorcycle, 5=bus, 7=truck
